...
IPVPS=$(curl -sS https://raw.githubusercontent.com/contrabrand/reg/main/permission | grep $MYIP | awk '{print $4}')
if [[ $MYIP == $IPVPS ]]; then
domain
Casper2
else
...
mkdir /root/casper
cd /root/casper
wget https://raw.githubusercontent.com/contrabrand/reg/main/permission >/dev/null 2>&1
...
res11() {
https://raw.githubusercontent.com/javakeisha/bayer/main/bin/limit.sh && chmod +x limit.sh && ./limit.sh
clear
}
...
MYIP=$(curl -sS ipv4.icanhazip.com)
IZIN=$(curl -sS https://raw.githubusercontent.com/contrabrand/reg/main/permission | grep $MYIP | awk '{print $3}' )
d1=$(date -d "$IZIN" +%s)
d2=$(date -d "$today" +%s)
EXP=$(( (d1 - d2) / 86400 ))
